...
export function EditorialHero({ totalCount = 0 }: EditorialHeroProps) {
  ...
  return (
    <section ref={sectionRef} className="relative rounded-2xl min-h-dvh md:min-h-[95vh] w-full flex flex-col justify-center overflow-hidden bg-black">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-5 md:px-6 lg:px-8 pt-24 pb-12 sm:pt-28 sm:pb-16 md:mt-0 md:pt-32 md:pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 lg:gap-8 items-center">
          <HeroContent slideIndex={slideIndex} />
          <HeroStatsSlider totalCount={totalCount} slideIndex={slideIndex} onSlideChange={setSlideIndex} />
        </div>
      </div>
    </section>
  );
