  ...
  return (
    <section className="py-16 sm:py-24 md:py-32 bg-primary text-primary-foreground relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-5 md:px-6 lg:px-8 max-w-7xl relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="mb-12 sm:mb-16 md:mb-24 text-center max-w-2xl mx-auto">
          <Heading variant="h2" className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight font-dynamic mb-4 sm:mb-6">
            {language === "bn" ? "মানুষের মতামত" : "Voices of Impact"}
          </Heading>
          <p className="text-primary-foreground/80 text-base sm:text-lg">{language === "bn" ? "যাঁরা আমাদের কাজ অনুসরণ করছেন, তাঁদের দৃষ্টিতে আমাদের উদ্যোগ।" : "What advocates and researchers are saying about our tracking platform."}</p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-12 max-w-5xl mx-auto">
          {testimonials.map((item, index) => (
            <motion.div key={item.id} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6, delay: index * 0.2, ease: [0.16, 1, 0.3, 1] }} className="bg-black/20 border border-white/10 p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-3xl relative">
              <Quote className="w-12 h-12 text-white/20 absolute top-8 right-8" />
              <p className="text-lg sm:text-xl md:text-2xl font-medium leading-relaxed mb-6 sm:mb-8 relative z-10 text-white/90">&quot;{item.quote}&quot;</p>
              <div>
