...
function getVictimStatusLabel(t: IncidentDetailTranslation, status: string): string {
  return t.common.incidents.victimStatus?.[status] ?? status;
}

export function IncidentVictims({ victims, language, victimsLabel, ageLabel, t }: IncidentVictimsProps) {
  return (
    <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="space-y-6">
      ...
      <div className="divide-y divide-border/30">
        {victims.map((victim) => {
          ...
          const victimBlocks = getDescriptionBlocksForLanguage(victim.description as { en?: unknown; bn?: unknown } | undefined, language);
          ...
          return (
            <motion.div key={victim._id} variants={fadeIn} className="py-6 first:pt-2 flex flex-col sm:flex-row gap-6">
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden bg-secondary/50 shrink-0 border border-border/50 flex items-center justify-center">{imageUrl ? <Image src={imageUrl} alt={victimName || (language === "bn" ? "ক্ষতিগ্রস্তের ছবি" : "Victim photo")} width={224} height={224} className="w-full h-full object-cover" unoptimized /> : <User className="w-8 h-8 sm:w-10 sm:h-10 text-secondary-foreground opacity-50" />}</div>
              <div className="flex-1 min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4 mb-2">
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground font-mono uppercase tracking-wider">
                    <span className="px-2 py-0.5 bg-muted rounded-full text-xs">{getVictimStatusLabel(t, victim.status)}</span>
                    {victim.age != null && (
                      <span>
                        {ageLabel}: {victim.age}
                      </span>
                ...
                {victimBlocks && victimBlocks.length > 0 ? (
                  <div className="text-xl text-muted-foreground leading-relaxed space-y-2">
                    <PortableText value={victimBlocks} />
                  </div>
                ) : (
                  victimDesc && (
                    <Text variant="muted" className="leading-relaxed text-xl">
                      {victimDesc}
                    </Text>
                  )
                )}
